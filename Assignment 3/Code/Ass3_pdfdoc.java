...
public class Crawler{
            ...
            static String file = "C:/Users/Admin/Desktop//5thSEM/WEB_TECH/Pdfdocs.xls";
            static HSSFWorkbook book = new HSSFWorkbook();
            static HSSFSheet sheet = book.createSheet("Pdflinks Worksheet");
    ...
    public static void fun(String URl,int row_cnt)throws IOException
    {
        ...
        for(Element url:urls)
        {
            if(!s.contains(url.attr("href")))
            {
                ...
                {
                    if(url.attr("href").toString().contains("pdf")||url.attr("href").toString().contains(".doc"))
                    {
                    s.add(url.attr("href"));
                    q.add(url.attr("href"));
                    row_cnt+=1;
                    row=sheet.createRow(row_cnt);
                    link=row.createCell(0);
                    url_cnt+=1;
        ...
        for(Element url:urls)
        {
            ...
            if(!s.contains(url.attr("href")))
            {
                if(url.attr("href").contains("http"))
                {
                    ...
                    link=row.createCell(0);
                    url_cnt+=1;
                    if(url.attr("href").contains("pdf")||url.attr("href").contains(".doc"))
                    {
                    link.setCellValue(url.attr("href"));
                    }
                    }
